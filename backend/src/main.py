...
def create_mock_user():
    ...
    user_df = pd.DataFrame.from_dict(
        [
            {
                "TookAndLiked": user1_took_and_liked,
                "TookAndNeutral": user1_took_and_neutral,
                "TookAndDisliked": user1_took_and_disliked,
                "Curious": user1_curious,
            }
        ]
    )
...
def main() -> None:
    ...
    user_courses_df, user_emb_list, encoder_for_user_courses, decoder_for_user_courses = create_mock_user()
    ...
    for concept_index in selected_recom_concepts:
        top_scores, top_courses = util.top_n_similarity_scores_for_concept(
            udemy_courses_df, sim_mat_concept_X_course, concept_index, n
        )
